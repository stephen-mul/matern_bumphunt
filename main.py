...
class ScanResult:
    ...
    predicted_bkg: np.ndarray  # GP predicted background in mask (counts)
...
def plot_results(
    ...
    """Produce the three-panel diagnostic plot."""
    ...
    fig = plt.figure(figsize=cfg.figsize, dpi=cfg.dpi, facecolor="white")
    gs = GridSpec(3, 1, height_ratios=[2.5, 1.2, 1.2], hspace=0.08)

    c_data = "#2c3e50"
    c_bkg = "#e74c3c"
    c_sig = "#3498db"
    c_thresh = "#e67e22"
    c_widths = plt.cm.viridis(np.linspace(0.2, 0.9, len(results)))

    # ── Panel 1: spectrum + GP background ─────────────────────────────────
    ax1 = fig.add_subplot(gs[0])
    ax1.bar(x, counts, width=1.0, color=c_data, alpha=0.45, label="Data")
    ax1.errorbar(
        x,
        counts,
        yerr=np.sqrt(counts + 0.5),
        fmt="none",
        ecolor=c_data,
        alpha=0.35,
        linewidth=0.7,
    ...
    # GP background (full-spectrum fit — sees all data incl. signal)
    ax1.plot(
        x_fine, bkg_mu, color=c_bkg, lw=2.0, label="GP background (full-spectrum fit)"
    )
    ax1.fill_between(
        x_fine, bkg_lo, bkg_hi, color=c_bkg, alpha=0.15, label="GP ±1sigma band"
    )
    ...
    ax1.set_title("Sliding-Window GP Bump Hunt", fontsize=13, fontweight="bold")
    ax1.tick_params(labelbottom=False)

    # ── Panel 2: per-width significance ───────────────────────────────────
    ...
    ax2.axhline(
        cfg.local_sigma_threshold,
        color=c_thresh,
        ls=":",
        lw=1.2,
        label=f"{cfg.local_sigma_threshold:.0f}sigma threshold",
    )
    ax2.axhline(0, color="gray", ls="-", lw=0.5, alpha=0.5)
    ax2.set_ylabel("Local signif. (sigma)")
    ax2.legend(loc="upper right", fontsize=8, ncol=3)
    ax2.tick_params(labelbottom=False)

    if true_signal_pos is not None:
        ax2.axvline(true_signal_pos, color=c_sig, ls="--", lw=1, alpha=0.5)

    # ── Panel 3: max significance envelope ────────────────────────────────
    ax3 = fig.add_subplot(gs[2], sharex=ax1)
    valid_env = ~np.isnan(max_sig)
    ax3.fill_between(pos[valid_env], 0, max_sig[valid_env], color=c_sig, alpha=0.25)
    ax3.plot(
        pos[valid_env],
        max_sig[valid_env],
        color=c_sig,
    ...
    ax3.axhline(cfg.local_sigma_threshold, color=c_thresh, ls=":", lw=1.2)
    ...
    if np.any(above & valid_env):
        peak_pos = pos[above & valid_env]
        peak_sig = max_sig[above & valid_env]
        ax3.scatter(
            peak_pos,
            peak_sig,
            color=c_thresh,
            s=30,
            zorder=5,
            label=f"Candidates (>{cfg.local_sigma_threshold:.0f}sigma)",
        )

    if true_signal_pos is not None:
        ax3.axvline(true_signal_pos, color=c_sig, ls="--", lw=1, alpha=0.5)

    ax3.set_xlabel("Bin index  (invariant mass proxy)")
    ax3.set_ylabel("Max local sigma")
